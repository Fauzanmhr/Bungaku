...
export const fetchUserByEmailOrID = async (data, isEmail = true) => {
    ...
    if (!isEmail)
        sql = 'SELECT `user_id` ,`fullname`, `email` FROM `users` WHERE `user_id`=?';
...
export default {
    ...
    login: async (req, res, next) => {
        try {
            ...
            const access_token = generateToken({ id: user.user_id });
            const refresh_token = generateToken({ id: user.user_id }, false);
            ...
            const [result] = await DB.execute(
                'INSERT INTO `refresh_tokens` (`token_id`,`token`) VALUES (?,?)',
                [user.user_id, md5Refresh]
            );
    ...
    getUser: async (req, res, next) => {
        try {
            ...
            // fetching user by the `user_id` (column)
            const user = await fetchUserByEmailOrID(data.user_id, false);
            if (user.length !== 1) {
    ...
    refreshToken: async (req, res, next) => {
        try {
            ...
            const access_token = generateToken({ id: data.user_id });
            const refresh_token = generateToken({ id: data.user_id }, false);
